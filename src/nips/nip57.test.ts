...
import { toContainEqual } from '../test-helpers.ts'
...
describe('getZapEndpoint', () => {
  ...
  test('returns null if fetch fails', async () => {
    ...
    expect(result).toBeNull()
    // Todo: bun:test does not have to HaveBeenCalledWith - is it necessary? Maybe the test can be done a different way?
...
describe('makeZapRequest', () => {
  ...
  test('returns a valid Zap request', () => {
    ...
    expect(toContainEqual(result.tags, ['p', 'profile']))
    expect(toContainEqual(result.tags, ['amount', '100']))
    expect(toContainEqual(result.tags, ['relays', 'relay1', 'relay2']))
    expect(toContainEqual(result.tags, ['e', 'event']))
  })
})
...
describe('makeZapReceipt', () => {
  test('returns a valid Zap receipt with a preimage', () => {
    ...
    expect(toContainEqual(result.tags, ['bolt11', bolt11])).toBeTruthy()
    expect(toContainEqual(result.tags, ['description', zapRequest])).toBeTruthy()
    expect(toContainEqual(result.tags, ['p', publicKey])).toBeTruthy()
  })
  ...
  test('returns a valid Zap receipt without a preimage', () => {
    ...
    expect(toContainEqual(result.tags, ['description', zapRequest])).toBeTruthy()
    expect(toContainEqual(result.tags, ['p', publicKey])).toBeTruthy()
    expect(toContainEqual(result.tags, 'preimage')).toBeFalsy()
  })
})